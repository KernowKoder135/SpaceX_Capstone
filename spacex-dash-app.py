...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig = px.pie(spacex_df, values='class', names='Launch Site', title='Total Launches at all sites.')
        return fig
    else:
        fig = px.pie(
            spacex_df[spacex_df['Launch Site'] == entered_site],
            names='class', title=f"Total Launch for {entered_site} only."
        )
        return fig
...
def get_scatter_chart(entered_site, payload_slider):
    if entered_site == "ALL":
        ...
        scatterplot = px.scatter(
            data_frame=filtered_data, x="Payload Mass (kg)", y="class", color="Booster Version Category")
        return scatterplot
    else:
